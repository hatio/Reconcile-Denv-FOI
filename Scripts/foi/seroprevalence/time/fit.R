...
parser = ArgumentParser(description = "Fit FOI from seroprevalence data.")
...
stanfile = file.path("Scripts/foi/seroprevalence", stanver,"model.stan")
...
source("Scripts/foi/prepare/seroprevalence.R")
...
if(!file.exists(stanRDS) | overwriteRDS){
    # presence matrices, for each individual (indiv x time)
    # encoding Kappa * time spent in the interval
    Kappa = 
        KappaInput %>%
        filter(Age < ageMax + 1) %>%
        with(
            ...
            , timeBirth = with(dat, year - age)
            , timeObs = with(dat, year)
            , SIMPLIFY = FALSE
        )) %>%
        do.call(what = rbind)
